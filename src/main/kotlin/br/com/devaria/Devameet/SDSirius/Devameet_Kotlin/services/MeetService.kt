...
import br.com.devaria.Devameet.SDSirius.Devameet_Kotlin.repositories.LastPositionsRepository
...
class MeetService(
    ...
    fun getMeetById(user: User, id: Long): Meet? {
        val meet = meetRepository.findById(id).orElse(null)
        return if (meet != null && meet.user?.id == user.id) {
            meet
        } else {
            null
        }
    ...
    fun delete (user: User, meetId:Long){
        ...
        if (meet != null && meet.user?.id == user.id){
            ...
            lastPositionsRepository.deleteAllLastPositionsByLink(meet.link)
